...
int android_reboot(int cmd, int flags, char *arg)
{
    int ret = 0;
    int reason = -1;
    int reboot_mode = 0, fd_lfs;

    /* update params.blk inside the j4fs partition to reflect the reboot mode */
    if (cmd == (int) ANDROID_RB_RESTART2) {
        if (arg && strcmp(arg, "recovery") == 0) {
            reboot_mode = 2; /* REBOOT_MODE_RECOVERY */
            cmd = ANDROID_RB_RESTART;
        }
    }
    if ((fd_lfs = open("/dev/block/mmcblk0p19", O_RDWR)) >= 0) {
        int pos = 0, found = 0;
        unsigned char read_buf[512];
        while(pos < 16777216) {
            if (read(fd_lfs, read_buf, sizeof(read_buf)) != sizeof(read_buf))
                break;
            /* PARAM_MAGIC = 0x72726624, PARAM_VERSION = 0x13
               we assume the params.blk file is aligned on 0x200 */
            if (memcmp(read_buf, "\x24\x66\x72\x72\x13\x00\x00\x00", 8) == 0) {
                found = 1;
                break;
            }
            pos += sizeof(read_buf);
        ...
        if (found) {
            *(int *)(read_buf + 0x54) = reboot_mode;
            lseek(fd_lfs, pos, SEEK_SET);
            write(fd_lfs, read_buf, sizeof(read_buf));
        }
        close(fd_lfs);
    }
